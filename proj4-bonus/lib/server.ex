defmodule Server do
  def startServer(totalTweets) do
    Task.start(fn -> Twitter.Server.start_link(totalTweets) end)
    # receive do
    #   {_} -> :ok
    # end
  end
...
defmodule Twitter.Server do
  ...
  def initialize_tables(totalTweets) do
    ...
    :ets.insert(:tweetsCount,{:tweetsCount,[0,0]})
  ...
  def startProcess() do
    receive do
      ...
      {:subscribeUser,userid,subscribeid,userpid} -> addSubscribeTo(subscribeid,userid)
                                               addSubscribers(userid,subscribeid,userpid)
  ...
  def addSubscribers(subscriberid,userid,userpid) do
    ...
    send userpid, {:receiveSubscriptionConfirmation,userid}
  end
