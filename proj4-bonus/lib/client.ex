defmodule Client do
  ...
  def startAutomateProcess(numClients,numMsg,parentpid) do
    ...
    subscribeToPid = zipfSubscribe(usersList)

    subscribersPid = getSubscribers(usersList)

    usersTweetsCount = Enum.map(usersTweetsCount,fn{userid,x}->
      ...
      numSubscribers = length(subscribersList)
      tweets = numMsg + (numSubscribers*2)
      tweetsCount = Enum.at(elem(Enum.at(:ets.lookup(:tweetsCount,:tweetsCount),0),1),1) + tweets
      :ets.insert(:tweetsCount,{:tweetsCount,[0,tweetsCount]})
      [userid,tweets]
    end)
  ...
  def rollOutTweets(usersTweetsCount, count, totalTweets,numMsg,parentpid) do
    ...
    if map_size(usersTweetsCount)!=0 do
  ...
  def zipfSubscribe(usersList) do
    ...
    IO.inspect constValue
    Enum.each(1..length(usersList),fn(x)->

      zipfSubs = constValue/x
      x="user"<>Integer.to_string(x)
      usersList = usersList -- [x]
      subscribeList = Enum.take_random(usersList, round(zipfSubs))
      Enum.each(subscribeList,fn(subscriber)->
        Twitter.Client.subscribe(String.to_atom(x),x,subscriber)
        Process.sleep(100)
      end)
...
defmodule Twitter.Client do
  ...
  def subscribe(actorName,userid,subscribeList) do
    GenServer.cast(actorName,{:subscribe,userid,subscribeList})
  end
  ...
  def handle_cast({:subscribe,userid,subscribeList},userid) do
    s_time = System.system_time(:microsecond)
    send :global.whereis_name(:ServerProcess) ,{:subscribeUser,userid,subscribeList,self()}
    receive do
      {:receiveSubscriptionConfirmation,subscriberid} -> IO.puts("#{subscriberid} subscribed to #{userid}\n")
    end
    tot_time = System.system_time(:microsecond) - s_time
    timeList = elem(Enum.at(:ets.lookup(:time,:timeList),0),1)
